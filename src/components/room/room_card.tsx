...
const RoomCard: React.FC<IProps> = ({ data }) => {
  // State to toggle the visibility of BookingForm
  ...
  const [status, setStatus] = useState(
    HOTEL_ROOMSTATUS_NAV.find((item) => item.name === data.status)
  );
  ...
  const handleClickOutside = (e: MouseEvent) => {
    ...
    if (
      popupStatusCleanRef.current &&
      !popupStatusCleanRef.current.contains(e.target as Node)
    ) {
    ...
    if (
      ...
      setShowPopupStatusClean(false); // Đóng popup khi click ngoài
    }
  };
  ...
  // Handler to toggle the state when "Nhận phòng nhanh" is clicked
  const handleBookingClick = () => {
    setShowBookingForm(!showBookingForm);
  };
  const handleCloseBookingForm = () => {
    setShowBookingForm(false); // Đóng form khi nhấn vào nút X
  };
  return (
    <section
      ref={roomCardRef}
    ...
    >
      ...
      <div className="p-3">
        <div className="flex mb-1 justify-between">
          {data.clean_status ? (
            <div className="center bg-[var(--ht-body-bg-)] rounded-full px-2 py-[3px] text-xs gap-1">
              <svg
            ...
            <div className="center bg-[var(--room-dirty-color-200-)] text-[var(--room-dirty-color-)] rounded-full px-2 py-[3px] text-xs gap-1">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="13"
                height="13"
          ...
          >
            ...
            {showPopupStatusClean &&
              (data.clean_status ? (
                <button className="popup-container !w-[70px] py-1 px-2 right-0 whitespace-nowrap text-xs hover:!bg-[var(--ht-neutral-100-)]">
                  Chưa dọn
                </button>
              ) : (
                <button className="popup-container !w-[70px] py-1 px-2 right-0 whitespace-nowrap text-xs hover:!bg-[var(--ht-neutral-100-)] ">
        ...
        <h1 className="text-[18px] font-medium text-black truncate">
          {data.name}
        </h1>
        {data.bookings && (
          <div className="flex absolute bottom-3">
            <div className="center time-check-out bg-opacity-3 text-white rounded-full px-2 py-[3px] text-xs gap-1">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="15"
                height="15"
              ...
              {handleFormattedDate(
                data.bookings[0].check_in_at,
                data.bookings[0].check_out_at
              )}
            </div>
          </div>
        )}
      ...
      {data.status === "Trống" ? (
        <UnusedRoomPopup
          ...
          handleBookingClick={handleBookingClick} // Pass the function here
        />
      ) : (
        <InusedRoomPopup
          ref={popupRef}
      ...
      {showBookingForm && (
        <BookingForm handleCloseBookingForm={handleCloseBookingForm} />
      )}
    </section>
  );
};
