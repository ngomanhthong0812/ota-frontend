  ...
  const handleSetStatusClean = async () => {
    try {
      const response = await axios.put(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/room/${data.id}`,
        JSON.stringify({
            ...
            {showPopupStatusClean &&
              (cleanStatus ? (
                <button
                  ...
                  className="popup-container !w-[70px] py-1 px-2 right-0 whitespace-nowrap text-xs hover:!bg-[var(--ht-neutral-100-)]"
                >
                  Chưa dọn
                </button>
              ) : (
