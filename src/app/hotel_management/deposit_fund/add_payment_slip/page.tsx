...
const AddPaymentSlipPage: React.FC<IProps> = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const reasonn = `${formData.content + ": " + formData.reason} `;

    try {
      // Gửi dữ liệu khi form được submit qua API
      const response = await axios.post(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/transaction/create/expense`,
        {
            ...
            <div className="flex-1 p-2 pb-5 border-x border-b border-[var(--ht-neutral-100-)] rounded-md">
              ...
              <div className="flex items-center gap-3 mb-4">
                ...
                >
                  ...
                  <option value="Chi từ phòng<">Chi từ phòng</option>
                  <optgroup label="Dịch vụ sinh hoạt">
                    <option value="--Điện">--Điện</option>
                    <option value="--Nước">--Nước</option>
                    <option value="--Internet">--Internet</option>
                    <option value="--Truyền hình cáp">--Truyền hình cáp</option>
                  ...
                  <option value="Trả nợ nhà cung cấp">
                    Trả nợ nhà cung cấp
                  </option>
                  <option value="Mua sắm đồ đạc">Mua sắm đồ đạc</option>
                  <option value="Chi tiêu cá nhân (Host)">
                    Chi tiêu cá nhân (Host)
                  </option>
                  <option value="Kết chuyển tiền (Host)">
                    Kết chuyển tiền (Host)
                  </option>
